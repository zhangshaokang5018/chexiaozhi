...
| 1 | A1-2 用户输入与发送 | [x] |
| 1 | A1-3 快捷问题区 | [x] |
| 1 | A1-4 接入 /api/chat（含 mock 开关） | [x] |
| 1 | A1-5 调度 Agent + steps 展示 | [x] |
...
- [x] 输入 "P0300" → 路由到 `dtc`
- [x] 输入 "咕噜咕噜响" → 路由到 `symptom`
- [x] 输入 "800 元换机油合理吗" → 路由到 `maintain`
- [x] 点 "拍报价单" → 路由到 `part`
- [x] 每条回复都能看到 Agent 名称 + 处理步骤
- [x] `USE_MOCK=true` 时整条聊天流程可脱离服务端演示
